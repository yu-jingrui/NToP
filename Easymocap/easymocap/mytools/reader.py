...
def read_keypoints3d(filename):
    ...
    for d_key, d_val in data.items():
        pid = d_val['id'] if 'id' in d_val.keys() else d_val['personID']
        ret = {'id': pid, 'type': 'body25'}
        for key in ['keypoints3d', 'handl3d', 'handr3d', 'face3d']:
            if key not in d_val.keys():continue
            pose3d = np.array(d_val[key], dtype=np.float32)
            if pose3d.shape[1] == 3:
...
def read_keypoints3d_all(path, key='keypoints3d', pids=[]):
    ...
    results = {}
    filenames = natsorted(glob(join(path, '*.json')))
