...
def convert_h36m_easymocap(H36M_ROOT, OUT_ROOT, seqs, cams, xml_path, debug=False):
    for seq in seqs:
        if debug:
            print('convert {}'.format(seq))
        # path with GT 3D pose
        pose_path = join(H36M_ROOT, 'extracted', seq, 'Poses_D3_Positions_mono')
        if debug:
            print(pose_path)
...
if __name__ == '__main__':
    ...
    xml_path = join(args.path,'H36M-Toolbox','metadata.xml')
    convert_h36m_easymocap(args.path, args.out, seqs, cams, xml_path, debug=args.debug)