...
def get_rays_from_KRT_adapted_from_original(H, W, K, R, T):
    r""" Sample rays on an image based on camera matrices (K, R and T)
    This function follows the same logic of the original function in HumanNerf
    1. step: list pixels in the image and convert them to CCS
    2. new step: undistort the pixels from equidistant projection to pinhole model (perspective projection)
    3. step: convert undistorted pixels into WCS
    4. step: calculate rays
    ...
    # convert pixel coordinates to CCS
    # X = K.X_c -> X_c = K^(-1).X = X.K^(-1)^T
    pixel_camera = np.dot(xy1, np.linalg.inv(K).T)

    # x, y values of the points
    pixel_xy = pixel_camera[:,:,:2]
    # rho = sqrt(x^2 + y^2), already in CCS so no need to subtract c_x and c_y
    rho = np.linalg.norm(pixel_xy, axis=-1)
    # rho = f.theta -> theta = rho / f
    # f = 1 in normalized coordinates, so
    theta = rho
    # map corners into infinity
    theta[theta > np.pi / 2.] = np.pi / 2.

    # undistorted magnitude of point vector in image plane
    r = np.tan(theta)
    # azimuth phi
    phi = np.arctan2(pixel_xy[:,:,1], pixel_xy[:,:,0])

    # * is equivalent to np.multiply, which is elementwise multiplication
    undistorted_pixel_camera = np.stack([r * np.cos(phi), r * np.sin(phi), np.ones_like(r)], axis=-1)
    undistorted_pixel_world = np.dot(undistorted_pixel_camera - T.ravel(), R)

    # calculate the ray direction
    rays_d = undistorted_pixel_world - rays_o[None, None]
